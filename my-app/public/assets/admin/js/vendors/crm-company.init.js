  ...
  document.body.contains(addEditModal) &&
    (addEditModal.addEventListener("show.bs.modal", function (e) {
      (count = contactList.items.length + 1),
        "edit" === action
          ? ((document.getElementById("add-edit-modal-label").innerHTML =
              "Edit Company"),
          ...
          e.target.closest("tr").children[1].innerText,
            ...
            t.get({ id: rowId }).forEach(function (e) {
              (currentRow = JSON.parse(JSON.stringify(e._values))).id ==
                ...
                (document.getElementById("view-detail-owner").innerHTML =
                  currentRow.owner),
                (document.getElementById("view-detail-company-name").innerHTML =
                  currentRow.company_name),
                (document.getElementById("view-detail-category").innerHTML =
                  currentRow.category),
                (document.getElementById("view-detail-rating").innerHTML =
                  currentRow.rating),
                (document.getElementById("view-detail-email-id").innerHTML =
                  currentRow.email_id),
                (document.getElementById("view-detail-location").innerHTML =
                  currentRow.location),
                (document.getElementById("view-logo-image").src =
                  currentRow.logo_img),
